...
class ASTModel(nn.Module):
    ...
    def __init__(self, label_dim=527, fstride=10, tstride=10, input_fdim=128, input_tdim=1024, imagenet_pretrain=True, audioset_pretrain=False, model_size='base384', verbose=True):
        ...
        # now load a model that is pretrained on both ImageNet and AudioSet
            ...
            #self.mlp_head_2 = nn.Sequential(nn.LayerNorm(self.original_embedding_dim), nn.Linear(self.original_embedding_dim, label_dim),
             ...
             #nn.Linear(label_dim, label_dim)                              )
            '''
            self.mlp_head_man = nn.Sequential(nn.Sigmoid(), nn.Linear(self.original_embedding_dim, label_dim, bias=False))
            many2one = torch.zeros(label_dim, self.original_embedding_dim)
            for i in range(label_dim-1): 
                many2one[i,i*38:(i+1)*38] = 0.026315
            many2one[label_dim-1, 760:] = 0.125
            with torch.no_grad():
                self.mlp_head_man[1].weight.data = many2one
            '''
            ...
            #self.re_cnn = nn.Sequential(nn.Conv2d(1, 1, kernel_size=(3, 3), stride=(1, 1), padding=1), nn.BatchNorm2d(1), nn.ReLU(inplace=True))
            #self.re_cnn_2 = nn.Sequential(nn.Conv2d(1, 1, kernel_size=(3, 3), stride=(1, 1), padding=1), nn.BatchNorm2d(1), nn.ReLU(inplace=True))
    ...
    def forward(self, x):
       ...
       # print(x.size(dim=2))128
        ...
        #print('x size before /2')
        #print(x.size(dim=0))#2
        #print(x.size(dim=1))#1214
        #print(x.size(dim=2))#768
        
        x = (x[:, 0] + x[:, 1]) / 2
        
        #manual mapping
        #layer_norm = nn.LayerNorm(self.original_embedding_dim)
        #x = layer_norm(x)
        #print('x size before mlp')
        #print(x.size(dim=0))#2
        #print(x.size(dim=1))#768
        
        #with torch.no_grad():
        #x = self.mlp_head_man(x)
        
        x = self.mlp_head(x)
        #print('x size after mlp')
        #print(x.size(dim=1))#20
        #x.cpu().numpy()
        return x
